...
struct VConfirm: View {
    private static let staticParameters = TripParametersC(
        tripId: "123",
        to: "Destination",
    ...
    init() {
        ...
        if let coords = VConfirm.staticParameters.fromCoords,
            ...
            ZStack {
                ...
                GeometryReader { geometry in
                    VStack(spacing: 20) {
                        ...
                        VStack(spacing: 8) {
                            Text("Pick-up: \(VConfirm.staticParameters.from ?? "Unknown Address")")
...
//                        .shadow(color: .white.opacity(0.1), radius: 5, x: -3, y: -3)
        ...
        var body: some View {
            GeometryReader { geometry in
                ...
                .frame(height: handleSize)
                 .padding()
                .background(
                NavigationLink("", destination: DropOff(), isActive: $navigate)
                    .opacity(0)
            )
            }
        }
    ...
    private func handleGoogleMaps() {
        let encodedAddress = (VConfirm.staticParameters.from ?? "Unknown Address").addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
...
struct VConfirm_Previews: PreviewProvider {
